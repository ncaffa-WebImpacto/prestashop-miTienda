...
{if $displayedFacets|count}
  <div id="search_filters" class="block search_filters">
    {block name='facets_title'}
      <p class="title_block">{l s='Filter By' d='Shop.Theme.Actions'}</p>
    {/block}
    <div class="block_content">
      {block name='facets_clearall_button'}
        {* if $activeFilters|count}
          <div id="_desktop_search_filters_clear_all" class="d-none d-md-block clear-all-wrapper">
            <button data-search-url="{$clear_all_link}" class="btn btn-secondary js-search-filters-clear-all mb-3">
      ...
      {foreach from=$displayedFacets item="facet"}
        <div class="facet clearfix facet-block">
          <p class="h6 facet-title d-none d-lg-block">{$facet.label}</p>
          ...
          <div class="title d-lg-none d-flex justify-content-between" data-target="#facet_{$_expand_id}" data-toggle="collapse"{if !$_collapse} aria-expanded="true"{/if}>
          ...
          {if in_array($facet.widgetType, ['radio', 'checkbox'])}
            {block name='facet_item_other'}
              <ul id="facet_{$_expand_id}" class="collapse show{if isset($facet.filters[0].properties.color) || isset($facet.filters[0].properties.texture)} color_facet{/if}">
                  ...
                  <li>
                    <label class="facet-label{if $filter.active} active {/if}" for="facet_input_{$_expand_id}_{$filter_key}">
                      {if $facet.multipleSelectionAllowed}
                        <span class="custom-control custom-checkbox{if isset($filter.properties.color) || isset($filter.properties.texture)} tip_inside{/if}">
                          <input
                      ...
                      {else}
                        <span class="custom-control custom-radio{if isset($filter.properties.color) || isset($filter.properties.texture)} tip_inside{/if}">
                          <input
                            id="facet_input_{$_expand_id}_{$filter_key}"
            ...
            {block name='facet_item_dropdown'}
              <div id="facet_{$_expand_id}" class="collapse show" data-collapse-hide-mobile>
                <select class="custom-select">
                  <option value="">{l s='(no filter)' d='Shop.Theme.Global'}</option>
                  {foreach from=$facet.filters item="filter"}
