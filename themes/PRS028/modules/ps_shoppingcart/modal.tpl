  ...
  <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title text-center" id="blockcart-modal-label"><i class="material-icons rtl-no-flip">&#xE876;</i>{l s='Product successfully added to your shopping cart' d='Shop.Theme.Checkout'}</h4>
      </div>
          ...
          <div class="col-lg-6 divide-right">
            <div class="media">
              ...
              <div class="media-body">
                ...
                <p class="product-qty">{$product.cart_quantity}&nbsp;x&nbsp;{$product.price}</p>
                {hook h='displayProductPriceBlock' product=$product type="unit_price"}
                {if $product.attributes}
                  <div class="product-attributes">
                    {foreach from=$product.attributes item="property_value" key="property"}
                      <span><strong>{l s='%label%:' sprintf=['%label%' => $property] d='Shop.Theme.Global'}</strong>: {$property_value}</span><br>
                    {/foreach}
                  </div>
                {/if}
              </div>
