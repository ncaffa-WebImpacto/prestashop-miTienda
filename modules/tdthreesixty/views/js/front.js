...
function product360Button() {
	$('.product-threesixty a').magnificPopup({
		...
		callbacks: {
			open: function() {
 				var spin = $(".threesixty"),
					dataImages360 = spin.data('images'),
					arrayImages360 = dataImages360.split(',');
				spin.ThreeSixty({
					totalFrames: spin.data('frames'),
					endFrame: spin.data('frames'),
					currentFrame: 1,
					imgList: ".threesixty_images",
					progress: ".spinner",
					imgArray: arrayImages360,
					responsive: true,
					navigation: true
				});
				$(window).resize();
			...
			close: function() {
				...
				spin.find('.threesixty_images').removeAttr('style').empty();
				spin.find('.nav_bar').remove();
			},
		},
	});
}
