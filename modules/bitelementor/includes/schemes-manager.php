...
class Schemes_Manager {
	...
	public function init() {
		...
		foreach ( self::$_schemes_types as $schemes_type ) {
			...
			$this->register_scheme( __NAMESPACE__ . '\Scheme_' . \BitElementorWpHelper::ucfirst( $schemes_type ) );
		}
	}

	public function register_scheme( $scheme_class ) {
		if ( ! class_exists( $scheme_class ) ) {
			return \BitElementorWpHelper::triggerWpError( 'scheme_class_name_not_exists' );
		}

		$scheme_instance = new $scheme_class();

		if ( ! $scheme_instance instanceof Scheme_Base ) {
			return \BitElementorWpHelper::triggerWpError( 'wrong_instance_scheme' );
		}
