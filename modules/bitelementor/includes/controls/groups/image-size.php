...
class Group_Control_Image_Size extends Group_Control_Base {
	...
	protected function _get_controls( $args ) {
		...
		$controls['size'] = [
			'label' => \BitElementorWpHelper::_x( 'Image Size', 'Image Size Control', 'elementor' ),
		...
		if ( isset( $image_sizes['custom'] ) ) {
			$controls['custom_dimension'] = [
				'label' => \BitElementorWpHelper::_x( 'Image Dimension', 'Image Size Control', 'elementor' ),
				'type' => Controls_Manager::IMAGE_DIMENSIONS,
				'description' => \BitElementorWpHelper::__( 'You can crop the original image size to any custom size. You can also set a single value for height or width in order to keep the original size ratio.', 'elementor' ),
				'condition' => [
					'size' => [ 'custom' ],
				],
				'separator' => 'none',
