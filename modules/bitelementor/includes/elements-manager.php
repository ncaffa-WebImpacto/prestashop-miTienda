...
class Elements_Manager {
    ...
    public function get_categories() {
        return [
            'basic' => [
                'title' => \BitElementorWpHelper::__( 'Elements', 'elementor' ),
                'icon' => 'font',
            ],
            'prestashop' => [
                'title' => \BitElementorWpHelper::__( 'Prestashop', 'elementor' ),
                'icon' => 'wordpress',
            ],
        ];
    }
    ...
    public function register_element( $element_class ) {
        if ( ! class_exists( $element_class ) ) {
            return \BitElementorWpHelper::triggerWpError('element_class_name_not_exists');
        }

        $element_instance = new $element_class();

        if ( ! $element_instance instanceof Element_Base ) {
            return \BitElementorWpHelper::triggerWpError('wrong_instance_element');
        }
