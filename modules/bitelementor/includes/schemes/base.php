...
abstract class Scheme_Base implements Scheme_Interface {
	...
	public function get_scheme_value() {
		$scheme_values = \BitElementorWpHelper::jsonDecode(\BitElementorWpHelper::get_option( 'elementor_scheme_' . static::get_type() ));

		if ( ! $scheme_values ) {
			$scheme_values = $this->get_default_scheme();

			\BitElementorWpHelper::update_option( 'elementor_scheme_' . static::get_type(), \BitElementorWpHelper::jsonEncode($scheme_values) );
		}
