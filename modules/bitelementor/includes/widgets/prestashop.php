...
class Widget_Prestashop extends Widget_Base
{
    ...
    private function _get_widget_instance()
    {
        if (is_null($this->_widget_instance)) {
            $this->_widget_instance = \BitElementorWpHelper::getBitElementorWidgetInstance($this->_widget_name);
        }
    ...
    protected function render($instance = [])
    {
        ...
        if (PluginElementor::instance()->editor->is_edit_mode()) {
            echo \BitElementorWpHelper::renderBitElementorWidgetPreview($this->_get_widget_instance()->id_base,
                $options);
        } else {
            echo \BitElementorWpHelper::renderBitElementorWidget($this->_get_widget_instance()->id_base, $options);
        }
    }
