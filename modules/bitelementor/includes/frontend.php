...
class Frontend {
	...
	public function print_css() {
		...
		$container_width = \BitElementorWpHelper::absint( \BitElementorWpHelper::get_option( 'elementor_container_width' ) );
		...
		if ( ! empty( $this->_enqueue_google_early_access_fonts ) ) {
			foreach ( $this->_enqueue_google_early_access_fonts as $current_font ) {
				printf( '<link rel="stylesheet" type="text/css" href="//fonts.googleapis.com/earlyaccess/%s.css">', \BitElementorWpHelper::strtolower( str_replace( ' ', '', $current_font ) ) );
			}
		...
		foreach ( $control['selectors'] as $selector => $css_property ) {
			...
			try {
				$output_css_property = preg_replace_callback( '/\{\{(?:([^.}]+)\.)?([^}]*)}}/', function( $matches ) use ( $control, $value_callback, $controls_stack, $value, $css_property ) {
					...
					$parsed_value = $control_obj->get_style_value( \BitElementorWpHelper::strtolower( $matches[2] ), $value_to_insert );



					if ($parser_control['name'] === 'background_image'){
						$parsed_value = \BitElementorWpHelper::getImage($parsed_value);
					}
