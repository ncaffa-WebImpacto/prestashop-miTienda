...
class AdminBitElementorController extends ModuleAdminController
{
    public $name;
    ...
    {
        $this->bootstrap = true;
        $this->className = 'BitElementorLanding';
        $this->table = 'bit_elementor_landing';
        ...
        $this->_orderBy = 'id_bit_elementor_landing';
        $this->identifier = 'id_bit_elementor_landing';
        $test = array();
        $test[0] = array(
            'id' => 0,
            ...
            'general' => array(
                ...
                'fields' =>    array(
                    'bit_homepage_layout' => array(
                        ...
                        'cast' => 'intval',
                        'type' => 'select',
                        'list' => array_merge($test, BitElementorLanding::getLandingPages()),
                        'identifier' => 'id'
                    ),
                    'bit_elementor_cache' => array(
        ...
        $this->fields_list = array(
            'id_bit_elementor_landing' => array('title' => $this->l('ID'), 'align' => 'center', 'class' => 'fixed-width-xs'),
    ...
    public function init()
    {
        if (Tools::isSubmit('edit' . $this->className)) {
            $this->display = 'edit';
        } elseif (Tools::isSubmit('addbit_elementor_landing')) {
            $this->display = 'add';
        }
    ...
    public function postProcess()
    {
        if (Tools::isSubmit('submit' . $this->className)) {
            ...
            Tools::redirectAdmin($this->context->link->getAdminLink('Admin'.$this->name) . '&id_bit_elementor_landing='.$returnObject->id .'&updatebit_elementor_landing');
        }
    ...
    {
        $landing = new BitElementorLanding((int) Tools::getValue('id_bit_elementor_landing'));

        if ($landing->id) {
            $url = $this->context->link->getAdminLink('BitElementorEditor').'&pageType=landing&pageId=' . $landing->id;
        } else {
            $url = false;
        }
        ...
        if (isset($landing->id)) {
            $helper->currentIndex = AdminController::$currentIndex . '&id_bit_link_block=' . $landing->id;
    ...
    protected function buildHelper()
    {
        ...
        $helper->override_folder = 'bitelementor/';
    ...
    {
        $this->toolbar_title[] = $this->l('Elementor - Page builder');
    }
