...
{if $products && count($products) >0}
				...
				<div class="thumbnail-container clearfix">
					<div class="thumbnail-inner">
						<div class="delete-wishlist-product clearfix">
							<a class="td-wishlist-button-delete btn" href="javascript:void(0)" title="{l s='Remove from this wishlist' mod='tdproductwishlist'}" data-id-wishlist="{$wishlist.id_wishlist}" data-id-wishlist-product="{$wishlist.id_wishlist_product}" data-id-product="{$product.id_product}"><i class="fa fa-trash"></i>
							</a>
						</div>
				...
				<div class="product-description">
					...
					<div class="wishlist-product-info">
						<div class="form-group">
							<label>{l s='Quantity' mod='tdproductwishlist'}</label>
						...
						<div class="form-group">
							<label>{l s='Priority' mod='tdproductwishlist'}</label>
							<select class="form-control wishlist-product-priority wishlist-product-priority-{$wishlist.id_wishlist_product}">
								{for $i=0 to 2}
									<option value="{$i}"{if $i == $wishlist.priority} selected="selected"{/if}>
										{if $i == 0}{l s='High' mod='tdproductwishlist'}{/if}
										{if $i == 1}{l s='Medium' mod='tdproductwishlist'}{/if}
										{if $i == 2}{l s='Low' mod='tdproductwishlist'}{/if}
									</option>
								{/for}
							</select>
						</div>
					...
					<div class="wishlist-product-action">
						<a class="wishlist-product-save-button btn btn-primary" href="javascript:void(0)" title="{l s='Save' mod='tdproductwishlist'}" data-id-wishlist="{$wishlist.id_wishlist}" data-id-wishlist-product="{$wishlist.id_wishlist_product}" data-id-product="{$product.id_product}">{l s='Save' mod='tdproductwishlist'}
						</a>
						{if isset($wishlists) && count($wishlists) > 0}
							<div class="dropdown td-wishlist-button-dropdown">
								<button class="td-wishlist-button dropdown-toggle btn btn-primary show-list" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{l s='Move' mod='tdproductwishlist'}</button>
								<div class="dropdown-menu td-list-wishlist td-list-wishlist-{$product.id_product}">
	...
	{/foreach}
{else}
	<div class="col-xl-12"><p class="alert alert-warning">{l s='No products' mod='tdproductwishlist'}</p></div>
{/if}
