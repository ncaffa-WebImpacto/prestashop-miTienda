...
class TdWishList extends ObjectModel
{
    ...
    {
        Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'tdwishlist_product` WHERE `id_wishlist` = '.(int)($this->id));
        if ($this->default) {
            $result = Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'tdwishlist` WHERE `id_customer` = '.(int)$this->id_customer.' and `id_wishlist` != '.(int)$this->id.' LIMIT 1');
            foreach ($result as $res) {
                Db::getInstance()->update('tdwishlist', array('default' => '1'), 'id_wishlist = '.(int)$res['id_wishlist']);
            }
        }
    ...
    public static function incCounter($id_wishlist)
    {
        ...
        return Db::getInstance()->execute('
            UPDATE `'._DB_PREFIX_.'tdwishlist` SET
    ...
    */
    ...
    public static function getProductByIdCustomer($id_wishlist, $id_customer, $id_lang, $id_product = null, $quantity = false)
    {
        ...
        $products = Db::getInstance()->executeS('
            ...
            FROM `'._DB_PREFIX_.'tdwishlist_product` wp
            ...
            LEFT JOIN `'._DB_PREFIX_.'tdwishlist` w ON w.`id_wishlist` = wp.`id_wishlist`
    ...
    public static function getInfosByIdCustomer($id_customer, $id_wishlist)
    {
        ...
        return (Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
            ...
            FROM `'._DB_PREFIX_.'tdwishlist_product` wp
            INNER JOIN `'._DB_PREFIX_.'tdwishlist` w ON (w.`id_wishlist` = wp.`id_wishlist`)
    ...
    public static function addProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute, $quantity)
    {
        ...
        $result = Db::getInstance()->getRow('
            SELECT wp.`quantity`
            FROM `'._DB_PREFIX_.'tdwishlist_product` wp
            JOIN `'._DB_PREFIX_.'tdwishlist` w ON (w.`id_wishlist` = wp.`id_wishlist`)
        ...
        if (empty($result) === false and sizeof($result)) {
            if (($result['quantity'] + $quantity) <= 0) {
                return (TdWishList::removeProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute));
            } else {
                return (Db::getInstance()->execute('
                    UPDATE `'._DB_PREFIX_.'tdwishlist_product` SET
            ...
            return (Db::getInstance()->execute('
                INSERT INTO `'._DB_PREFIX_.'tdwishlist_product` (`id_wishlist`, `id_product`, `id_product_attribute`, `quantity`, `priority`) VALUES(
                '.(int)($id_wishlist).',
                '.(int)($id_product).',
    ...
    public static function removeProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute)
    {
        ...
        $result = Db::getInstance()->getRow('
            ...
            FROM `'._DB_PREFIX_.'tdwishlist` w
            LEFT JOIN `'._DB_PREFIX_.'tdwishlist_product` wp ON (wp.`id_wishlist` = w.`id_wishlist`)
            WHERE `id_customer` = '.(int)($id_customer).'
        ...
        return Db::getInstance()->execute('
            DELETE FROM `'._DB_PREFIX_.'tdwishlist_product`
            WHERE `id_wishlist` = '.(int)($id_wishlist).'
            AND `id_product` = '.(int)($id_product).'
            AND `id_product_attribute` = '.(int)($id_product_attribute));
    }
    ...
    */
    ...
    {
        return (Bool)Db::getInstance()->getValue('SELECT * FROM `'._DB_PREFIX_.'tdwishlist` WHERE `id_customer` = '.(int)$id_customer.' AND `default` = 1');
    }

    public static function getDefault($id_customer)
    {
        return Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'tdwishlist` WHERE `id_customer` = '.(int)$id_customer.' AND `default` = 1');
    }

    /**
    * Set current TdWishList as default
    *
    * @return boolean
    */
    public function setDefault()
    {
        if ($default = $this->getDefault($this->id_customer)) {
            Db::getInstance()->update('tdwishlist', array('default' => '0'), 'id_wishlist = '.(int)$default[0]['id_wishlist']);
        }

        return Db::getInstance()->update('tdwishlist', array('default' => '1'), 'id_wishlist = '.(int)$this->id);
    }
