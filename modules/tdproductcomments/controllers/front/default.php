...
/* Include Module */
include_once(dirname(__FILE__).'/../../tdproductcomments.php');
/* Include Models */
include_once(dirname(__FILE__).'/../../classes/TdProductComment.php');
include_once(dirname(__FILE__).'/../../classes/TdProductCommentCriterion.php');

class TdProductCommentsDefaultModuleFrontController extends ModuleFrontController
{
    ...
    {
        $module_instance = new TdProductComments();
        ...
        if (!$this->context->customer->id && !Configuration::get('TDPRODUCTCOMMENT_ALLOW_GUESTS')) {
        ...
        if (!count($errors)) {
            $customer_comment = TdProductComment::getByCustomer(Tools::getValue('id_product'), $id_customer, true, $id_guest);
            if (!$customer_comment || ($customer_comment && (strtotime($customer_comment['date_add']) + (int)Configuration::get('TDPRODUCTCOMMENT_MINIMAL_TIME')) < time())) {
                $comment = new TdProductComment();
                ...
                foreach (Tools::getValue('criterion') as $id_product_comment_criterion => $grade) {
                    $grade_sum += $grade;
                    $product_comment_criterion = new TdProductCommentCriterion($id_product_comment_criterion);
                ...
                $result = true;
                Tools::clearCache(Context::getContext()->smarty, $this->getTemplatePath('tdproductcomments_reviews.tpl'));
            } else {
                $result = false;
                $errors[] = $module_instance->l('Please wait before posting another comment', 'default').' '.Configuration::get('TDPRODUCTCOMMENT_MINIMAL_TIME').' '.$module_instance->l('seconds before posting a new comment', 'default');
            }
        } else {
            $result = false;
        }
    ...
    protected function ajaxProcessReportAbuse()
    {
        ...
        if (TdProductComment::isAlreadyReport(Tools::getValue('id_product_comment'), $this->context->cookie->id_customer)) {
            die('0');
        }

        if (TdProductComment::reportComment((int)Tools::getValue('id_product_comment'), $this->context->cookie->id_customer)) {
            die('1');
        }
    ...
    protected function ajaxProcessCommentIsUsefull()
    {
        ...
        if (TdProductComment::isAlreadyUsefulness(Tools::getValue('id_product_comment'), $this->context->cookie->id_customer)) {
            die('0');
        }

        if (TdProductComment::setCommentUsefulness((int)Tools::getValue('id_product_comment'), (bool)Tools::getValue('value'), $this->context->cookie->id_customer)) {
            die('1');
        }
