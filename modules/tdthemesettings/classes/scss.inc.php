...
class scssc
{
    ...
    */
    ...
    protected function mergeMediaTypes($type1, $type2)
    {
        ...
        $t1 = '';
        if (count($type1) > 1) {
            $m1= Tools::strtolower($type1[0]);
            $t1= Tools::strtolower($type1[1]);
        } else {
            $t1 = Tools::strtolower($type1[0]);
        }
        $m2 = '';
        $t2 = '';
        if (count($type2) > 1) {
            $m2 = Tools::strtolower($type2[0]);
            $t2 = Tools::strtolower($type2[1]);
        } else {
            $t2 = Tools::strtolower($type2[0]);
        }
        if (($m1 == 'not') ^ ($m2 == 'not')) {
            if ($t1 == $t2) {
                return null;
        ...
        foreach ($this->importPaths as $dir) {
            if (is_string($dir)) {
                ...
                foreach ($urls as $full) {
                    $full = $dir . (!empty($dir) && Tools::substr($dir, -1) != '/' ? '/' : '') . $full;
...
class scss_parser
{
    ...
    */
    ...
    {
        $comment[1] = Tools::substr(preg_replace(array('/^\s+/m', '/^(.)/m'), array('', ' \1'), $comment[1]), 1);
    ...
    */
    protected function to($what, &$out, $until = false, $allowNewline = false)
    {
        ...
        if ($until) {
            $this->count -= Tools::strlen($what); /* give back $what */
        }
        $out = $m[1];
        return true;
    }
    ...
    {
        return 1 + substr_count(Tools::substr($this->buffer, 0, $pos), "\n");
    }
    ...
    */
    protected function matchString(&$m, $delim)
    {
        ...
        $match = Tools::substr($this->buffer, $this->count, $end - $this->count);
        $m = array(
            $match . $token,
            $match,
            $token
        );
        $this->count = $end + Tools::strlen($token);
...
class scss_formatter_compressed extends scss_formatter
{
    ...
    public function blockLines($inner, $block)
    {
        ...
        foreach ($block->lines as $index => $line) {
            if (Tools::substr($line, 0, 2) === '/*' && Tools::substr($line, 2, 1) !== '!') {
                unset($block->lines[$index]);
            } elseif (Tools::substr($line, 0, 3) === '/*!') {
                $block->lines[$index] = '/*' . Tools::substr($line, 3);
            }
        }
...
class scss_server
{
    ...
    */
    ...
    {
        switch (true) {
            case Tools::getValue('p'):
                return Tools::getValue('p');
            case isset($_SERVER['PATH_INFO']):
                return $_SERVER['PATH_INFO'];
            case isset($_SERVER['DOCUMENT_URI']):
                return Tools::substr($_SERVER['DOCUMENT_URI'], Tools::strlen($_SERVER['SCRIPT_NAME']));
        }
    }
    ...
    */
    ...
    {
        if (($input = $this->inputName()) && strpos($input, '..') === false && Tools::substr($input, -5) === '.scss') {
    ...
    */
    protected function needsCompile($in, $out, &$etag)
    {
        ...
        if (is_readable($metadataName)) {
            $metadata = unserialize(Tools::file_get_contents($metadataName));
    ...
    protected function getIfModifiedSinceHeader()
    {
        ...
        if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE'])) {
            ...
            if (false !== ($semicolonPos = strpos($modifiedSince, ';'))) {
                $modifiedSince = Tools::substr($modifiedSince, 0, $semicolonPos);
            }
        }
    ...
    */
    protected function compile($in, $out)
    {
        ...
        $css     = $this->scss->compile(Tools::file_get_contents($in), $in);
    ...
    */
    public function serve($salt = '')
    {
        ...
        if ($input = $this->findInput()) {
            ...
            header('Last-Modified: ' . $lastModified);
            echo Tools::file_get_contents($output);
            return;
        }
