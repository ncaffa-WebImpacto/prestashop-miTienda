...
include_once _PS_MODULE_DIR_ . 'bitmegamenu/classes/BitMenuTab.php';
include_once _PS_MODULE_DIR_ . 'bitmegamenu/classes/BitMenuHtml.php';
include_once _PS_MODULE_DIR_ . 'bitmegamenu/classes/BitMenuLinks.php';


class BitMegaMenu extends Module implements WidgetInterface
{
    ...
    {
        $this->name = 'bitmegamenu';
        $this->tab = 'front_office_features';
        $this->version = '1.0.0';
        $this->author = 'ThemeDelights';
        $this->need_instance = 0;
        $this->bitdevmode = false;
        $this->bitTheme = false;
        $this->bootstrap = true;

        parent::__construct();

        $this->displayName = $this->l('BIT - Megamenu');
        $this->description = $this->l('Drag and Drop Mega Menu by ThemeDelights');

        $this->ps_versions_compliancy = array('min' => '1.7', 'max' => _PS_VERSION_);

        $this->config_name = 'bitmegamenu';
        ...
        $this->verticalPosition = Configuration::get('tdopt_vm_position');
    }
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'bitmegamenu_tabs_shop` (
        ...
        $res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'bitmegamenu_tabs` (
				`id_tab` int(10) unsigned NOT NULL AUTO_INCREMENT,
...
');
        ...
        $res &= Db::getInstance()->execute('
	CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'bitmegamenu_tabs_lang` (
...
');
        ...
        /* custom links */
        $res &= Db::getInstance()->execute('
	CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'bitmenulinks` (
		`id_bitmenulinks` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
');
        ...
        /* custom links lang */
        $res &= Db::getInstance()->execute('
	CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'bitmenulinks_lang` (
		`id_bitmenulinks` INT(11) UNSIGNED NOT NULL,
		`id_lang` INT(11) UNSIGNED NOT NULL,
		`id_shop` INT(11) UNSIGNED NOT NULL,
		`label` VARCHAR( 128 ) NOT NULL ,
		`link` VARCHAR( 128 ) NOT NULL ,
		INDEX ( `id_bitmenulinks` , `id_lang`, `id_shop`)
		) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=UTF8;
');

        /* custom html */
        $res &= Db::getInstance()->execute('
	CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'bitmegamenu_html` (
...
');
        ...
        $res &= Db::getInstance()->execute('
	CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'bitmegamenu_htmlc` (
...
');
        ...
        /* custom html lang */
        $res &= Db::getInstance()->execute('
	CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'bitmegamenu_htmlc_lang` (
...
');
    ...
    protected function deleteTables()
    {
        return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'bitmegamenu_tabs_shop`, `' . _DB_PREFIX_ . 'bitmegamenu_tabs`, `' . _DB_PREFIX_ . 'bitmegamenu_tabs_lang`, `' . _DB_PREFIX_ . 'bitmenulinks`, `' . _DB_PREFIX_ . 'bitmenulinks_lang`, `' . _DB_PREFIX_ . 'bitmegamenu_html`, `' . _DB_PREFIX_ . 'bitmegamenu_htmlc`, `' . _DB_PREFIX_ . 'bitmegamenu_htmlc_lang`;
			');
    }
    ...
    public function getContent()
    {
        ...
        $this->context->controller->addJS($this->_path . 'views/js/jquery.jqColorPicker.min.js');
        ...
        Media::addJsDef(array(
            'bitsearch_url' => $this->context->link->getAdminLink('AdminModules', true) . '&configure=' . $this->name,
            'bitBaseUrl'  => Tools::safeOutput($base_url)
        ));
        ...
        if (Tools::isSubmit('addTab') || (Tools::isSubmit('id_tab') && !Tools::isSubmit('submitAddTab') && BitMenuTab::tabExists((int)Tools::getValue('id_tab')))) {
            ...
            $this->generateCss();
            $this->clearMenuCache();
        } elseif (Tools::isSubmit('addCustomHtml') || (Tools::isSubmit('id_html') && !Tools::isSubmit('submitAddHtml') && BitMenuHtml::htmlExists((int)Tools::getValue('id_html')))) {
            ...
            $this->generateCss();
            $this->clearMenuCache();
        } elseif (Tools::isSubmit('submitBlocktopmenuLinks') || (Tools::isSubmit('updatebitmenulinks') && Tools::getValue('updatelink'))) {
            ...
            if ($count_links_label || $count_label) {
                if (!$count_links_label) {
                    ...
                    foreach ($shops as $shop_id) {
                        if (Tools::isSubmit('updatebitmenulinks')) {
                            $added = BitMenuLinks::update($links_label, $labels, (int)$shop_id,
                                Tools::getValue('id_bitmenulinks'), Tools::getValue('new_window', 0));
                        } else {
                            $added = BitMenuLinks::add($links_label, $labels, (int)$shop_id,
            ...
            $this->clearMenuCache();
        } elseif (Tools::isSubmit('deletebitmenulinks')) {
            $errors_delete_link = array();
            $id_bitmenulinks = Tools::getValue('id_bitmenulinks', 0);
            $shops = Shop::getContextListShopID();

            foreach ($shops as $shop_id) {
                $deleted = BitMenuLinks::remove($id_bitmenulinks, (int)$shop_id);
                Configuration::updateValue($this->config_name . '_mobile_menu',
                    str_replace(array('LNK' . $id_bitmenulinks . ',', 'LNK' . $id_bitmenulinks), '',
    ...
    private function _postValidation()
    {
        ...
        if (Tools::isSubmit('submitAddTab')) {
            ...
            if (Tools::isSubmit('id_tab')) {
                if (!Validate::isInt(Tools::getValue('id_tab')) && !BitMenuTab::tabExists(Tools::getValue('id_tab'))) {
        ...
        } elseif (Tools::isSubmit('delete_id_tab') && (!Validate::isInt(Tools::getValue('delete_id_tab')) || !BitMenuTab::tabExists((int)Tools::getValue('delete_id_tab')))) {
            $errors[] = $this->l('Invalid id_tab');
        }
    ...
    private function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitAddTab')) {
            ...
            if (Tools::getValue('id_tab')) {
                $tab = new BitMenuTab((int)Tools::getValue('id_tab'));
                ...
                $tab->position = BitMenuTab::getNextPosition(Tools::getValue('menu_type'));
            }
            ...
            $id_shop_list = Tools::getValue('checkBoxShopAsso_bitmegamenu_tabs');
            ...
            $tab = new BitMenuTab((int)Tools::getValue('delete_id_tab'));
            $res = $tab->delete();
            $this->clearMenuCache();
            if (!$res) {
    ...
    private function _postValidationHtml()
    {
        ...
        if (Tools::isSubmit('submitAddHtml')) {
            if (Tools::isSubmit('id_html')) {
                if (!Validate::isInt(Tools::getValue('id_html')) && !BitMenuHtml::htmlExists(Tools::getValue('id_html'))) {
        ...
        } elseif (Tools::isSubmit('delete_id_html') && (!Validate::isInt(Tools::getValue('delete_id_html')) || !BitMenuHtml::htmlExists((int)Tools::getValue('delete_id_html')))) {
            $errors[] = $this->l('Invalid id_html');
        }
        if (count($errors)) {
            $this->_html .= $this->displayError(implode('<br />', $errors));
    ...
    private function _postProcessHtml()
    {
        ...
        if (Tools::isSubmit('submitAddHtml')) {
            if (Tools::getValue('id_html')) {
                $tab = new BitMenuHtml((int)Tools::getValue('id_html'));
                if (!Validate::isLoadedObject($tab)) {
                    $this->_html .= $this->displayError($this->l('Invalid id_tab'));
            ...
            $tab = new BitMenuHtml((int)Tools::getValue('delete_id_html'));
            $res = $tab->delete();
            $this->clearMenuCache();
            if (!$res) {
                $this->_html .= $this->displayError('Could not delete.');
    ...
    public function renderAddForm()
    {
        ...
        if (Tools::isSubmit('id_tab') && BitMenuTab::tabExists((int)Tools::getValue('id_tab'))) {
            $tab = new BitMenuTab((int)Tools::getValue('id_tab'));
    ...
    public function renderAddHtmlForm()
    {
        ...
        if (Tools::isSubmit('id_html') && BitMenuHtml::htmlExists((int)Tools::getValue('id_html'))) {
                    ...
                    switch ($node['contentType']) {
                        case 1:
                            if (isset($node['content']['ids'])) {
                                $customhtml = new BitMenuHtml((int)$node['content']['ids'], $id_lang);
    ...
    protected function renderForm()
    {
        ...
        $fields_form_submenutabs = array(
            'form' => array(
                ...
                'legend' => array(
                    'title' => $this->l('Predefined submenu tabs'),
                    'icon' => 'icon-cogs',
                ),
                'input' => array(
                    array(
        ...
        $fields_form_html = array(
            'form' => array(
                ...
                'legend' => array(
                    'title' => $this->l('Predefined custom html content'),
                    'icon' => 'icon-cogs',
                ),
                'input' => array(
                    array(
        ...
        $helper->identifier = $this->identifier;
        $helper->submit_action = 'submitbitmegamenuModule';
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false)
            . '&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name;
        $helper->token = Tools::getAdminTokenLite('AdminModules');

        if (Tools::getIsset('updatebitmenulinks') && !Tools::getValue('updatebitmenulinks')) {
            $fields_form_custom['form']['submit'] = array(
                'name' => 'updatebitmenulinks',
                'title' => $this->l('Update'),
            );
        }
        ...
        if (Tools::isSubmit('updatebitmenulinks')) {
            ...
            $fields_form_custom['form']['input'][] = array('type' => 'hidden', 'name' => 'id_bitmenulinks');
        }
    ...
    protected function getConfigFormValues()
    {
        ...
        if (Tools::getIsset('updatebitmenulinks') && (int)Tools::getValue('id_bitmenulinks') > 0) {
            $var['updatelink'] = true;
            $var['id_bitmenulinks'] = (int)Tools::getValue('id_bitmenulinks');

            $link = BitMenuLinks::getLinkLang((int)Tools::getValue('id_bitmenulinks'), $id_shop);
    ...
    public function generateCss($allShops = false)
    {
        ...
        $tabs = array();
        $tabsV = array();
        $tabs = BitMenuTab::getTabsFrontend(1, true);
        $tabsV = BitMenuTab::getTabsFrontend(2, true);
        ...
        $tabs = BitMenuTab::getTabsFrontend(3, true);
        ...
        if ($allShops) {
            ...
            foreach ($shops as $shop) {
                $myFile = $this->local_path . "views/css/bitmegamenu_" . (int)$shop->id . ".css";
                file_put_contents($myFile, $css);
            }
            self::clearAssetsCache();
        } else {
            if (Shop::getContext() == Shop::CONTEXT_SHOP) {
                $myFile = $this->local_path . "views/css/bitmegamenu_" . (int)$this->context->shop->getContextShopID() . ".css";
    ...
    public function hookHeader()
    {
        ...
        if (Shop::getContext() == Shop::CONTEXT_SHOP) {
            $this->context->controller->registerStylesheet('modules-' . $this->name . '-style-custom',
                'modules/' . $this->name . '/views/css/bitmegamenu_' . (int)$this->context->shop->getContextShopID() . '.css',
    ...
    public function renderWidget($hookName = null, array $configuration = [])
    {
        ...
        } elseif ((preg_match('/^displayMegamenu\d*$/', $hookName))) {
    ...
    public function renderSelectedTabsSelect($tabs)
    {
        ...
        if (Tools::strlen($tabs)) {
            ...
            foreach ($tabs as $tab_id) {
                $tab = new BitMenuTab($tab_id, $id_lang, $id_shop);
    ...
    {
        $custom_html = array();
        $custom_html = BitMenuHtml::getHtmls();
        return $custom_html;
    }
    ...
    public function renderChoicesSelect($single = false, $name = null, $class = null, $mobile = false)
    {
        ...
        $links = BitMenuLinks::gets($this->context->language->id, (int)Shop::getContextShopID(), null);
        foreach ($links as $link) {
            if ($link['label'] == '') {
                $default_language = Configuration::get('PS_LANG_DEFAULT');
                $link = BitMenuLinks::get($link['id_bitmenulinks'], $default_language, (int)Shop::getContextShopID());
                if (!in_array('LNK' . (int)$link[0]['id_bitmenulinks'], $items)) {
                    $html .= '<option value="LNK' . (int)$link[0]['id_bitmenulinks'] . '">' . $spacer . Tools::safeOutput($link[0]['label']) . '</option>';
                }
            } elseif (!in_array('LNK' . (int)$link['id_bitmenulinks'], $items)) {
                $html .= '<option value="LNK' . (int)$link['id_bitmenulinks'] . '">' . $spacer . Tools::safeOutput($link['label']) . '</option>';
            }
        }
        $html .= '</optgroup>';
        $html .= '</select>';
    ...
    private function makeMenuOptionMobile()
    {
        ...
        foreach ($menu_item as $item) {
            ...
            switch (Tools::substr($item, 0, Tools::strlen($values[1]))) {
                ...
                case 'LNK':
                    $link = BitMenuLinks::get((int)$id, (int)$id_lang, (int)$id_shop);
                    if (count($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = BitMenuLinks::get($link[0]['id_bitmenulinks'], (int)$default_language,
                                (int)Shop::getContextShopID());
                        }
                        $html .= '<option selected="selected" value="LNK' . (int)$link[0]['id_bitmenulinks'] . '">' . Tools::safeOutput($link[0]['label']) . '</option>';
                    }
                    break;
    ...
    {
        $tabs = array();
        $tabs = BitMenuTab::getTabs($menu_type);

        $this->context->smarty->assign(
            array(
                'link' => $this->context->link,
                'tabs' => $tabs,
                'bitdevmode' => $this->bitdevmode,
                'menu_type' => $menu_type,
            )
        );
    ...
    public function renderListCustomLinks()
    {
        ...
        foreach ($shops as $shop_id) {
            $links = array_merge($links, BitMenuLinks::gets((int)$this->context->language->id, (int)$shop_id, null));
        }

        $fields_list = array(
            'id_bitmenulinks' => array(
        ...
        $helper->identifier = 'id_bitmenulinks';
        $helper->table = 'bitmenulinks';
    ...
    private function makeMegaMenu($menu_type)
    {
        ...
        $tabs = BitMenuTab::getTabsFrontend($menu_type, false);
            ...
            if ($tab['submenu_type'] == 1) {
                if (Tools::strlen(($tab['submenu_content']))) {
                    ...
                    foreach ($tab['submenu_content'] as $tab_id) {
                        $innertab = new BitMenuTab($tab_id, $id_lang, $id_shop);
        ...
        switch (Tools::substr($item, 0, Tools::strlen($value[1]))) {
            ...
            case 'LNK':
                ...
                if (count($link)) {
                    if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                        $default_language = Configuration::get('PS_LANG_DEFAULT');
                        $link = BitMenuLinks::get($id, $default_language, (int)Shop::getContextShopID());
                    }
    ...
    private function installSamples()
    {
        ...
        $tab = new BitMenuTab();
        $tab->menu_type = 1;
        $tab->position = BitMenuTab::getNextPosition(1);
        ...
        foreach ($languages as $language) {
            $tab->title[$language['id_lang']] = 'Contact';
        }
        $tab->add();
    }
    ...
    public function ajaxProcessUpdateHorizontalTabsPosition()
    {
        ...
        foreach ($tabs as $position => $id_tab) {
            $res = Db::getInstance()->execute('
            UPDATE `' . _DB_PREFIX_ . 'bitmegamenu_tabs` SET `position` = ' . (int)$position . '
    ...
    public function ajaxProcessupdateVerticalTabsPosition()
    {
        $tabs = Tools::getValue('tabs');
        foreach ($tabs as $position => $id_tab) {
            $res = Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'bitmegamenu_tabs` SET `position` = ' . (int)$position . '
