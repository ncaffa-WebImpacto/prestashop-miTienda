...
class BitMenuTab extends ObjectModel
{
    public $id;
    public $menu_type;
    public $id_tab;
    ...
    {
        Shop::addTableAssociation('bitmegamenu_tabs', array('type' => 'shop'));
    ...
    {
        $res = true;
        $tab = new BitMenuTab((int) $this->id);
        $res &= $this->reOrderPositions($tab->menu_type);
        $res &= Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'bitmegamenu_tabs_shop`
    ...
    {
        if (isset($this->id)) {
            Db::getInstance()->delete('bitmegamenu_tabs_shop', 'id_tab = ' . (int) $this->id);
        }
        $this->associateTo($this->id_shop_list);
    ...
    public static function getTabs($menu_type)
    {
        ...
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT hs.`id_tab` as id_tab, hssl.`title`, hss.`position`
			FROM ' . _DB_PREFIX_ . 'bitmegamenu_tabs_shop hs
			LEFT JOIN ' . _DB_PREFIX_ . 'bitmegamenu_tabs hss ON (hs.id_tab = hss.id_tab)
			LEFT JOIN ' . _DB_PREFIX_ . 'bitmegamenu_tabs_lang hssl ON (hss.id_tab = hssl.id_tab)
    ...
    public static function getNextPosition($menu_type)
    {
        ...
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('SELECT MAX(hss.`position`) AS `next_position`
			FROM `' . _DB_PREFIX_ . 'bitmegamenu_tabs` hss, `' . _DB_PREFIX_ . 'bitmegamenu_tabs_shop` hs
    ...
    public static function getAllShopTabs()
    {
        ...
        foreach ($rows as $id => $tab) {
            $tabs[$tab['id_tab']] = new BitMenuTab($tab['id_tab']);
    ...
    public function reOrderPositions($menu_type)
    {
        ...
        $rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT hss.`position` as position, hss.`id_tab` as id_tab
			FROM `' . _DB_PREFIX_ . 'bitmegamenu_tabs` hss
			LEFT JOIN `' . _DB_PREFIX_ . 'bitmegamenu_tabs_shop` hs ON (hss.`id_tab` = hs.`id_tab`)
			WHERE hs.`id_shop` = ' . (int) $id_shop . ' AND hss.`menu_type` = ' . (int) $menu_type . ' AND hss.`position` > ' . (int) $this->position);

        foreach ($rows as $row) {
            $current_tab = new BitMenuTab($row['id_tab']);
