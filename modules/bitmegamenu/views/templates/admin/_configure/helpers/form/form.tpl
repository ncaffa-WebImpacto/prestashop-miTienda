...
$(document).ready(function() {
	$('.iframe-upload').fancybox({
		...
		beforeClose: function() {
			...
			$input.val(val.replace(bitBaseUrl, ""));
		}
	});
	...
	function add()
	{
		$("#availableItems option:selected").each(function(i){
			...
			if (val == "PRODUCT") {
				val = prompt('{l s='Indicate the ID number for the product' mod='bitmegamenu' js=1}');
				if (val == null || val == "" || isNaN(val))
					return;
				text = '{l s='Product ID #' mod='bitmegamenu' js=1}'+val;
				val = "PRD"+val;
			}
	...
	function move(up)
	{
		...
		if (tomove.length >1) {
			alert('{l s='Please select just one item' mod='bitmegamenu'}');
			return false;
		}
		if (up)
...
{block name="fieldset"}
	...
	{block name="label"}
		{if ($input.type == 'grid_creator') || ($input.type == 'docs') }
		{else}
			{$smarty.block.parent}
		{/if}
	{/block}
...
{block name="input_row"}
	{if !isset($input.bitTheme) || !$input.bitTheme}
		{if isset($input.hide) && $input.hide}<div style="display: none !important;">{/if}
			{if isset($input.preffix_wrapper)}<div id="{$input.preffix_wrapper}" {if isset($input.wrapper_hidden) && $input.wrapper_hidden} class="hidden clearfix"{/if}>{/if}
				{if isset($input.accordion_wrapper)}<a class="btn btn-primary menu-collapse-expand" data-toggle="collapse" href="#{$input.accordion_wrapper}" aria-expanded="false" aria-controls="{$input.accordion_wrapper}">{l s='Expand submenu optional design options' mod='bitmegamenu'}
...
{block name="input"}
	{if $input.type == 'link_choice'}
		<div class="row">
			<div class="col-lg-1">
				<h4 style="margin-top:5px;">{l s='Change position' mod='bitmegamenu'}</h4>
			...
			<div class="col-lg-4">
				<h4 style="margin-top:5px;">{l s='Selected items' mod='bitmegamenu'}</h4>
				{*HTML CONTENT*} {$selected_links nofilter}
			</div>
			<div class="col-lg-4">
				<h4 style="margin-top:5px;">{l s='Available items' mod='bitmegamenu'}</h4>
		...
		<div class="row">
			<div class="col-lg-1"></div>
				<div class="col-lg-4"><a href="#" id="removeItem" class="btn btn-default"><i class="icon-arrow-right"></i> {l s='Remove' mod='bitmegamenu'}</a></div>
				<div class="col-lg-4"><a href="#" id="addItem" class="btn btn-default"><i class="icon-arrow-left"></i> {l s='Add' mod='bitmegamenu'}</a></div>
			</div>
	{elseif $input.type == 'tabs_choice'}
			...
			<div class="col-lg-4">
				<h4 style="margin-top:5px;">{l s='Selected tabs' mod='bitmegamenu'}</h4>
				{*HTML CONTENT*} {$selected_tabs nofilter}
			</div>
			<div class="col-lg-4">
				<h4 style="margin-top:5px;">{l s='Available predefined tabs' mod='bitmegamenu'}</h4>
	...
	{elseif $input.type == 'image_upload'}
		...
		<a href="filemanager/dialog.php?type=1&field_id={$input.name}" class="btn btn-default iframe-upload"  data-input-name="{$input.name}" type="button">{l s='Select image' mod='bitmegamenu'} <i class="icon-angle-right"></i></a>
	{elseif $input.type == 'docs'}
		<p>Please read file <a href="{$module_path}documentation.pdf" target="_blank">documentation.pdf</a> to configure this module.</p>
	...
	{elseif $input.type == 'icon_selector'}
		<div class="input-group col-lg-3">
			...
			<span class="input-group-addon">{l s='Select icon' mod='bitmegamenu'}</span>
		</div>
	{elseif $input.type == 'border_generator'}
		<div class="col-xs-2">
			<select name="{$input.name}_type" id="{$input.name}_type">
				<option value="5" {if $fields_value[$input.name].type==5}selected{/if}>{l s='groove' mod='bitmegamenu'}</option>
				<option value="4" {if $fields_value[$input.name].type==4}selected{/if}>{l s='double' mod='bitmegamenu'}</option>
				<option value="3" {if $fields_value[$input.name].type==3}selected{/if}>{l s='dotted' mod='bitmegamenu'}</option>
				<option value="2" {if $fields_value[$input.name].type==2}selected{/if}>{l s='dashed' mod='bitmegamenu'}</option>
				<option value="1" {if $fields_value[$input.name].type==1}selected{/if}>{l s='solid' mod='bitmegamenu'}</option>
				<option value="0" {if $fields_value[$input.name].type==0}selected{/if}>{l s='none' mod='bitmegamenu'}</option>
			</select>
		</div>
		<div class="col-xs-2">
	...
	{elseif $input.type == 'color'}
		<div class="form-group">
			<div class="col-lg-2">
				<div class="row">
					<div class="input-group">
						<input class="tdcolor" data-default="" data-serializable="true" name="{$input.name}" value="{$fields_value[$input.name]|escape:'html':'UTF-8'}" type="text" />
	...
	{elseif $input.type == 'custom_info'}
		&nbsp; {l s='Entire design configuration you can find in TD - Theme Settings > Menu section' mod='bitmegamenu'}
	{else}
		{$smarty.block.parent}
	{/if}
{/block}
