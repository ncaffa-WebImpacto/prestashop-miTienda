	...
	$.fn.PavMegaMenuList = function(opts) {
		...
		this.each(function() {
	 		$(".quickedit",this).click( function(){
	 			location.href=config.action+"&id_bitblogcat="+$(this).attr('rel').replace("id_","");
	 		} );

	 		$(".quickdel",this).click( function(){
	 			if( confirm(config.confirm_del) ){
	 				location.href=config.action+"&dodel=1&id_bitblogcat="+$(this).attr('rel').replace("id_","");
	 			}
...
jQuery(document).ready(function(){
 	...
 	$(".bit-modal-action, #widgets a.btn").fancybox({
	 	'type':'iframe',
	 	'width':950,
	 	'height':500,
	 	afterLoad:function(   ){
	...
	if ($('#image_link-images-thumbnails').length > 0)
	{
		...
		bitblog_del_img($('#image_link-images-thumbnails'), 'image');
	}

	if ($('#thumb_link-images-thumbnails').length > 0)
	{

		bitblog_del_img($('#thumb_link-images-thumbnails'), 'thumb');
	}

	$('.bitblog-del-img-bt').click(function(){
		if( confirm(bitblog_del_img_mess) ){
...
function bitblog_del_img(img_id_element, img_name_e)
{
	img_id_element.append('<a class="btn btn-default bitblog-del-img-bt" href="#" data-element="'+img_name_e+'" data-id="'+img_id_element.attr('id')+'"><i class="icon-trash"></i>'+bitblog_del_img_txt+'</a>');
}
...
jQuery(document).ready(function(){
	...
	var id_panel = $("#bloggeneralsetting .bitblog-globalconfig li.active a").attr("href");
	$(id_panel).addClass('active').show();
	$('.bitblog-globalconfig li').click(function(){
		if(!$(this).hasClass('active'))
		{
			var default_tab = $(this).find('a').attr("href");
			$('#BITBLOG_DASHBOARD_DEFAULTTAB').val(default_tab);
		}
	})
});
