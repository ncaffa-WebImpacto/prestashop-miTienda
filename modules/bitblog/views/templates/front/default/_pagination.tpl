...
{if isset($p) AND $p}
	...
	<div class="row">
		<nav class="pagination">
			<div class="col-xs-12 col-md-6 col-lg-6 d-flex justify-content-md-start justify-content-center">
				{if $nb_items > 1}
					{l s='Showing' mod='bitblog'} {$blogShowingStart} - {$blogShowing} {l s='of' mod='bitblog'} {$nb_items} {l s='items' mod='bitblog'}
				{else}
					{l s='Showing' mod='bitblog'} {$blogShowingStart} - {$blogShowing} {l s='of' mod='bitblog'} 1 {l s='item' mod='bitblog'}
				{/if}
			</div>
			{if $start!=$stop}
				<div id="pagination{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="col-xs-12 col-md-6 col-lg-6">
					<ul class="page-list clearfix justify-content-md-end justify-content-center m-md-0 mt-2">
						{if $p != 1}
							...
							<li id="pagination_previous{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}">
								<a {$no_follow_text|escape:'html':'UTF-8'} class="previous" rel="prev" href="{$link->goPage($requestPage, $p_previous)|escape:'html':'UTF-8'}">
									...
									<span class="sr-only">{l s='Previous' mod='bitblog'}</span>
								</a>
							</li>
						{else}
							<li id="pagination_previous{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}">
						...
						{if $pages_nb > 1 AND $p != $pages_nb}
							...
							<li id="pagination_next{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}">
								<a {$no_follow_text|escape:'html':'UTF-8'} class="next" rel="next" href="{$link->goPage($requestPage, $p_next)|escape:'html':'UTF-8'}">
									<span class="sr-only">{l s='Next' mod='bitblog'}</span>
									<i class="material-icons">&#xE315;</i>
								</a>
							</li>
						{else}
