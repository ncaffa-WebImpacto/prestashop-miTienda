...
class AdminBitblogCategoriesController extends ModuleAdminController
{
    public $name = 'bitblog';
    ...
    {
        $this->bootstrap = true;
        $this->id_bitblogcat = true;
        $this->table = 'bitblogcat';

        $this->className = 'bitblogcat';
        $this->lang = true;
    ...
    public function setMedia($isNewTheme = false)
    {
        ...
        $this->context->controller->addJS(__PS_BASE_URI__.'modules/bitblog/views/js/admin/jquery.nestable.js');
    ...
    public function postProcess()
    {
        ...
        if (Tools::getValue('doupdatepos') && Tools::isSubmit('updatePosition')) {
            ...
            foreach ($child as $id_parent => $menus) {
                ...
                foreach ($menus as $id_bitblogcat) {
                    $sql = 'UPDATE `'._DB_PREFIX_.'bitblogcat` SET `position` = '.(int)$i.', id_parent = '.(int)$id_parent.'
                            WHERE `id_bitblogcat` = '.(int)$id_bitblogcat;
        ...
        if (Tools::getValue('dodel')) {
            $obj = new bitblogcat((int)Tools::getValue('id_bitblogcat'));
            $res = $obj->delete();
        ...
        if (Tools::isSubmit('save'.$this->name) && Tools::isSubmit('active')) {
            if ($id_bitblogcat = Tools::getValue('id_bitblogcat')) {
                # validate module
                $megamenu = new bitblogcat((int)$id_bitblogcat);
            } else {
                # validate module
                $megamenu = new bitblogcat();
                $megamenu->randkey = BitBlogHelper::genKey();
            }
            ...
            if ($megamenu->validateFields(false) && $megamenu->validateFieldsLang(false)) {
                ...
                if (isset($_FILES['image_link']) && isset($_FILES['image_link']['tmp_name']) && !empty($_FILES['image_link']['tmp_name'])) {
                    ...
                    if (ImageManager::validateUpload($_FILES['image_link'])) {
                        ...
                        return false;
                    } elseif (!ImageManager::resize($tmp_name, _PS_THEME_DIR_.'assets/img/modules/bitblog/'.$id_shop.'/c/'.$_FILES['image_link']['name'])) {
                        return false;
                    }
                    unlink($tmp_name);
                    $megamenu->image = $_FILES['image_link']['name'];
                    if (isset($old_image)) {
                        unlink(_PS_THEME_DIR_.'assets/img/modules/bitblog/'.$id_shop.'/c/'.$old_image);
                    }

                    $megamenu->save();
                }
                Tools::redirectAdmin(AdminController::$currentIndex.'&savebitblog&token='.Tools::getValue('token').'&id_bitblogcat='.$megamenu->id);
            } else {
                // validate module
    ...
    public function renderList()
    {
        ...
        $output .= '<div class="col-md-4"><div class="panel panel-default"><h3 class="panel-title">'.$this->l('Tree Blog Categories Management').'</h3>'
                .'<div class="panel-content">'.$this->l('To sort orders or update parent-child, you drag and drop expected menu.')
            ...
            foreach ($f['form']['input'] as $j => $input) {
                ...
                if (isset($obj->{trim($input['name'])})) {
                    ...
                    if ($input['name'] == 'image_link' && $obj->image != '') {
                        ...
                        $thumb = $url._THEME_DIR_.'assets/img/modules/bitblog/'.$id_shop.'/c/'.$obj->image;
