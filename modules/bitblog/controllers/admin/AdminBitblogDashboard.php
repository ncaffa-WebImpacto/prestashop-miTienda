...
class AdminBitblogDashboardController extends ModuleAdminController
{
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('saveConfiguration')) {
            $keys = BitBlogHelper::getConfigKey(false);
            $post = array();
            foreach ($keys as $key) {
                # validate module
            ...
            $multi_lang_keys = BitBlogHelper::getConfigKey(true);
            ...
            BitBlogConfig::updateConfigValue('cfg_global', Tools::jsonEncode($post));
            // print_r(Tools::getValue('BITBLOG_DASHBOARD_DEFAULTTAB'));

            Configuration::updateValue('BITBLOG_DASHBOARD_DEFAULTTAB', Tools::getValue('BITBLOG_DASHBOARD_DEFAULTTAB'));
            // print_r(Configuration::updateValue('BITBLOG_DASHBOARD_DEFAULTTAB'));
            // die();
        }
    }
    ...
    public function renderView()
    {
        ...
        $quicktools[] = array(
            ...
            'href' => $link->getAdminLink('AdminBitblogCategories'),
            'icon' => 'icon-desktop',
            'class' => '',
        );
        ...
        $quicktools[] = array(
            'title' => $this->l('Blogs'),
            'href' => $link->getAdminLink('AdminBitblogBlogs'),
            'icon' => 'icon-list',
            'class' => '',
        );
        ...
        $quicktools[] = array(
            ...
            'href' => $link->getAdminLink('AdminBitblogBlogs').'&addbitblog_blog',
            'icon' => 'icon-list',
            'class' => '',
        );
        ...
        $quicktools[] = array(
            ...
            'href' => $link->getAdminLink('AdminBitblogComments'),
            'icon' => 'icon-list',
            'class' => '',
        );
        ...
        $templates = BitBlogHelper::getTemplates();
        $url_rss = Tools::htmlentitiesutf8('http://'.$_SERVER['HTTP_HOST'].__PS_BASE_URI__).'modules/bitblog/rss.php';
        $form = '';
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'hidden',
                    'name' => 'BITBLOG_DASHBOARD_DEFAULTTAB',
                    'default' => '',
                ),
                array(
                    'type' => 'select',
...
//                ),
        ...
        $data = BitBlogConfig::getConfigValue('cfg_global');
        $obj = new stdClass();
        ...
        $comments = BitBlogComment::getComments(null, 10, $this->context->language->id);
        $blogs = BitBlogBlog::getListBlogs(null, $this->context->language->id, 0, 10, 'hits', 'DESC');

        $template->assign(array(
            'doc_url' => __PS_BASE_URI__.'modules/bitblog/documentation.pdf',
            'quicktools' => $quicktools,
            'showed' => 1,
            'comment_link' => $link->getAdminLink('AdminBitblogComments'),
            'blog_link' => $link->getAdminLink('AdminBitblogBlogs'),
            'blogs' => $blogs,
            'count_blogs' => BitBlogBlog::countBlogs(null, $this->context->language->id),
            'count_cats' => Bitblogcat::countCats(),
            'count_comments' => BitBlogComment::countComments(),
            'latest_comments' => $comments,
            'globalform' => $form,
            'default_tab' => Configuration::get('BITBLOG_DASHBOARD_DEFAULTTAB')
        ));
        // print_r(Configuration::get('BITBLOG_DASHBOARD_DEFAULTTAB'));die();
    ...
    public function getConfigFieldsValues($obj)
    {
        ...
        $fields_values['BITBLOG_DASHBOARD_DEFAULTTAB'] = Tools::getValue('BITBLOG_DASHBOARD_DEFAULTTAB', Configuration::get('BITBLOG_DASHBOARD_DEFAULTTAB'));
